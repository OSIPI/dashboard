...
3. Update package version, citation, CodeMeta and UTC-dated categorized changelog together. Preserve previous notes. Bun lockfile and Zenodo metadata have no version field to update.
...
5. Create `chore(release): vX.Y.Z` containing only the four metadata files, then an annotated tag binding archive checksum and source SHA. Signing and commit/push hooks are disabled to avoid hidden side effects. App SHA identifies tested source parent; release commit adds metadata/notes.
