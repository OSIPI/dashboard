...
4. Reject MRI samples in local static inputs, then run `bun run check`, `bun test` and Python companion unittest discovery. Build with release version and source parent SHA and verify the output is sample-free. Include `release.json` identifying version, source and `/dashboard` base path. Freeze `osipy-vX.Y.Z.tar.gz` plus `.sha256` in Git's `dashboard-releases/vX.Y.Z/` directory, outside disposable `build/`, and inspect the archive before checkpointing it. Record its SHA-256 before any publication.
...
8. The atomic push of the release commit to `main` triggers the independent Pages workflow. It checks out that exact SHA, installs locked dependencies, builds with `APP_SHA` set to the pushed SHA, verifies the build is sample-free, uploads the Pages artifact and deploys only after that build succeeds. The tagged archive and checksum remain sample-free GitHub Release assets; keep published tags protected. GitHub's repository-level immutable releases may additionally be enabled by maintainers, but tooling never changes repository settings.
